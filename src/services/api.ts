...
const getMostPopularMovies = () => {
    return instance.get(`/en/API/MostPopularMovies/${process.env.REACT_APP_IMDB_KEY}`)
}

const getMovieById = (id: string) => {
    return instance.get(`https://imdb-api.com/en/API/Title/${process.env.REACT_APP_IMDB_KEY}/${id}/FullActor,FullCast,Posters,Images,Trailer,Ratings`)
}

const searchMoviesByExpression = (expression: string) => {
    return instance.get(`https://imdb-api.com/en/API/SearchMovie/${process.env.REACT_APP_IMDB_KEY}/${expression}`)
}
